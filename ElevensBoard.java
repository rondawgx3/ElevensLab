...
public class ElevensBoard extends Board {
	...
	private List<Integer> findJQK(List<Integer> selectedCards) {
		...
		for (Integer kObj : selectedCards) {
			int k = kObj.intValue();
			if (cardAt(k).rank().equals("jack") && jackCount==0) {
				jQK.add(kObj);
				jackCount++;
			} else if (cardAt(k).rank().equals("queen") && queenCount==0) {
				jQK.add(kObj);
				queenCount++;
			} else if (cardAt(k).rank().equals("king") && kingCount==0) {
				jQK.add(kObj);
				kingCount++;
			}
		}
	...
	private boolean playPairSum11IfPossible() {
		/* *** TO BE IMPLEMENTED IN ACTIVITY 11 *** */
		List<Integer> cIndexes = cardIndexes();
		List<Integer> pairList = findPairSum11(cIndexes);
		if(pairList.size()==2) {
	...
	private boolean playJQKIfPossible() {
		/* *** TO BE IMPLEMENTED IN ACTIVITY 11 *** */
		List<Integer> cIndexes = cardIndexes();
		List<Integer> jQKList = findJQK(cIndexes);
		if(jQKList.size()==3) {
